...
export default function DespesasDashboard({ globalBuffer }: Props) {
  ...
  if (rows.length === 0) {
    return <section className="glass-panel p-6">Não encontrei dados na aba DESPESAS.</section>;
  }
      ...
      <section className="glass-panel p-6">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-white/60 mt-1">Resumo por mês e ano da aba DESPESAS.</p>
          </div>
          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead className="text-white/60 border-b border-white/10">
              <tr>
                ...
                <th className="text-right py-3">A pagar</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((r, idx) => (
                <tr key={idx} className="border-b border-white/5">
                  <td className="py-3 pr-4 font-medium">{r.fornecedor || '—'}</td>
                  <td className="py-3 pr-4">{r.grupo || '—'}</td>
                  <td className="py-3 pr-4">{r.item}</td>
                  <td className="py-3 pr-4">{r.status}</td>
                  <td className="py-3 pr-4 text-right">{moneyBR(r.total)}</td>
                  <td className="py-3 text-right">{moneyBR(r.aPagar)}</td>
                </tr>
              ))}
            </tbody>
          </table>
